"""FactorMuE, with Gaussian + softmax approximation for u and r,
not Dirichlet."""
...
def FactorMuE(latent_dims, latent_length, latent_alphabet_size, alphabet_size,
              ...
              l_conc, z_distr='Normal', dtype=tf.float64):
...
def build_FactorMuE_variational(latent_dims, latent_length, max_delete,
                                latent_alphabet_size, alphabet_size,
    ...
    enc_transfer_mats = mue.make_transfer(padded_data_length-1, max_delete,
                                          dtype=dtype)
...
def train(dataset, dataset_train, dataset_test, padded_data_length,
          latent_dims, latent_length, max_delete, latent_alphabet_size,
          alphabet_size, bt_scale, b0_scale, u_conc, r_conc, l_conc, z_distr,
    ...
    FactorMuE_variational, trainable_variables = build_FactorMuE_variational(
            latent_dims, latent_length, max_delete, latent_alphabet_size,
            alphabet_size, u_conc, r_conc, l_conc, padded_data_length,
            z_distr=z_distr, dtype=dtype)
    ...
    heldout_perplex, heldout_likelihood = evaluate_loop(
                      ...
                      l_conc, z_distr, mc_samples, dtype, writer)
...
def single_alignment_mode(FactorMuE_variational, x, xlen, latent_dims,
    ...
    for rep in range(mc_samples):
        ...
        with condition(z=qz.distribution.sample(),
                       ...
                       r=qr.distribution.sample(),
                       l=ql.distribution.sample()):
            posterior_predictive = FactorMuE(
                    latent_dims, latent_length,
                    latent_alphabet_size, alphabet_size,
                    xlen, transfer_mats, bt_scale, b0_scale,
        ...
        pmode = posterior_predictive.distribution.posterior_mode(x)
...
def main(config):
    ...
    (FactorMuE_variational, trainable_variables,
     heldout_perplex, heldout_likelihood, embed_mean, embed_std) = train(
               ...
               latent_dims, latent_length, max_delete, latent_alphabet_size,
               alphabet_size, bt_scale, b0_scale, u_conc, r_conc, l_conc,
               z_distr, anneal_epochs, max_epochs, shuffle_buffer,
               batch_size, optimizer_name, learning_rate, mc_samples, dtype,
               writer, out_folder)
